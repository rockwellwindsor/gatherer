...
RSpec.describe Project do
  ...
  describe 'estimates' do
    ...
    it "stubs an object again" do
      project = Project.new(name: "Project Bluelight")
      allow(project).to receive(:name).and_return("Fred")
      expect(project.name).to eq("Fred")
    end

    it "stubs the class" do
      allow(Project).to receive(:find).and_return(Project.new(name: "Project YellowLight"))
      project = Project.find(1)
      expect(project.name).to eq("Project YellowLight")
    end

    # it "mocks and object" do
    #   mock_project = Project.new(name: "Project RedLight")
    #   expect(mock_project).to receive(:name).and_return("Fred")
    #   expect(mock_project).to eq("Fred")
    # end
  end
end
